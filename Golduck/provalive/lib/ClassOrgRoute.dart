...
Future<Map<String, List<atleta>>> fetchClasses(String raceid, String org) async {
  ...
  if (response.statusCode == 200) {
    ...
    List<atleta> atleti = List<atleta>.from((fetched).map((e) => atleta(e["name"],e["surname"],e["org"],e["position"],e["time"],e["class"],e["numero"])));
...
_buildExpandableContent(List<atleta>? lista) {
  ...
  for (atleta a in lista!)
    columnContent.add(
      differenze.contains(a.numero)? Container(color: Colors.red.shade100,child: AthleteTile(a)) : AthleteTile(a)
    );
